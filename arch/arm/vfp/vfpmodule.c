 ...
 *
 * IRQ pipeline/Dovetail: always enters with hard irqs off, bh
 * disabled if running inband. We make sure to read the fpregs we need
 * before re-enabling hard irqs, at which point an oob task might
 * preempt. Returns with hard irqs ON, which matches the converse flip
 * in vfp_entry().
 */
void VFP_bounce(u32 trigger, u32 fpexc, struct pt_regs *regs)
{
	...
	hard_cond_local_irq_enable(); /* Matches converse in vfp_entry(). */
 ...
 exit:
	/*
	 * IRQ pipeline: we might have switched from oob to inband
	 * context upon fp exception, in which case we did not disable
	 * bh on entry.
	 */
...
{
	unsigned long flags;
	unsigned int cpu = hard_get_cpu_bh(flags);
			  ...
			  u32 resume_return_address)
{
	...
	/*
	 * IRQ pipeline: vfp_support_entry() re-enables hard irqs and
	 * bh (if required) before returning to us, via either
	 * local_bh_enable_and_ret or VFP_bounce().
	 */
