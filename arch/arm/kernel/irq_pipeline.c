...
static void create_ipi_domain(void)
{
	 ...
	 * numbers starting from OOB_IPI_BASE onward. The sirqs
	 * obtained can be injected into the pipeline upon IPI receipt
	 * like other interrupts.
	 */
	sipic_domain = irq_domain_add_simple(NULL, NR_IPI, OOB_IPI_BASE,
...
{
	enum ipi_msg_type msg = ipi - OOB_IPI_BASE;
...
void do_IRQ_pipelined(struct irq_desc *desc)
{
	 ...
	 * code.
	 */
	if (irq >= OOB_IPI_BASE && irq < OOB_IPI_BASE + NR_IPI) {
