...
/* Catches upstream changes to rcu_exit semantics. */
struct rcu_exit_state {
#define IDTENTRY_RCU_EXIT	BIT(0)
#define IDTENTRY_INBAND_STALLED	BIT(1)
#define IDTENTRY_OOB_ENTRY	BIT(2)
	int val;
};

#define rcu_exit_code(__val)	((struct rcu_exit_state){ .val = (__val) })

void idtentry_enter_user(struct pt_regs *regs);
void idtentry_exit_user(struct pt_regs *regs);

struct rcu_exit_state idtentry_enter_cond_rcu(struct pt_regs *regs);
void idtentry_exit_cond_rcu(struct pt_regs *regs, struct rcu_exit_state rcu_exit);
...
{									\
	struct rcu_exit_state rcu_exit = idtentry_enter_cond_rcu(regs);	\
									\
...
#ifdef CONFIG_X86_LOCAL_APIC
# ifdef CONFIG_X86_UV
DECLARE_IDTENTRY_SYSVEC_PIPELINED(UV_BAU_MESSAGE,		sysvec_uv_bau_message);
# endif
