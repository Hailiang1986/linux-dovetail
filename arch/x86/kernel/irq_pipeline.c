...
static struct rcu_exit_state pipeline_enter_rcu(void)
{
	if (!IS_ENABLED(CONFIG_TINY_RCU) && is_idle_task(current)) {
		rcu_irq_enter();
		return rcu_exit_code(IDTENTRY_RCU_EXIT);
	}

	rcu_irq_enter_check_tick();

	return rcu_exit_code(0);
}

static void pipeline_exit_rcu(struct rcu_exit_state rcu_exit)
{
...
void arch_do_IRQ_pipelined(struct irq_desc *desc)
{
	...
	rcu_exit = pipeline_enter_rcu();
	irq_enter_rcu();

	if (desc->irq_data.domain == sipic_domain)
		run_on_irqstack_cond(do_sysvec_inband, desc, NULL);
	else
		run_on_irqstack_cond(desc->handle_irq, desc, NULL);

	irq_exit_rcu();
	pipeline_exit_rcu(rcu_exit);
...
kernel_exit_check_downgrade(struct pt_regs *regs)
{
	...
	return rcu_exit_code(IDTENTRY_RCU_EXIT|IDTENTRY_INBAND_STALLED);
}
