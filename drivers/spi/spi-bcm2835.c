...
{
	struct bcm2835_spi *bs = spi_controller_get_devdata(ctlr);
	unsigned long cdiv, spi_used_hz;
	unsigned long hz_per_byte, byte_limit;
	u32 cs = bs->prepare_cs[spi->chip_select];

	cdiv = bcm2835_get_clkdiv(bs, tfr->speed_hz, &spi_used_hz);
					...
					struct spi_oob_transfer *xfer)
{
	...
	cdiv = bcm2835_get_clkdiv(bs, xfer->setup.speed_hz, &spi_used_hz);
	xfer->effective_speed_hz = spi_used_hz;
	bcm2835_wr(bs, BCM2835_SPI_CLK, cdiv);
